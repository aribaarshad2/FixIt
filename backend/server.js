...
app.get('/api/seed-temp', async (req, res) => {
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const user = await User.create({ name: names[i], email: `${names[i].toLowerCase().replace(/ /g, '.')}@fixit.com`, password: hashedPassword, role: 'user', phone: `03${Math.floor(100000000 + Math.random() * 900000000)}` });
